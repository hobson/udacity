...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        noisy_alpha = (motion[0]+random.gauss(0,self.steering_noise))%(2.*pi) 
        noisy_d = (motion[1]+random.gauss(0,self.distance_noise))
        
        beta = (noisy_d/self.length)*tan(noisy_alpha)
        if beta<.001:
            self.x += noisy_d*cos(self.orientation)
            self.y += noisy_d*sin(self.orientation)
        else:
            R = self.length/tan(noisy_alpha)
            cx = self.x - sin(self.orientation)*R
            cy = self.y + cos(self.orientation)*R
            self.x = cx + sin((self.orientation+beta)%(2.*pi))*R
            self.y = cy - cos((self.orientation+beta)%(2.*pi))*R
    ...
    # according to
    #           self.bearing_noise
    def sense(self, flg=1): #do not change the name of this function
        #print flg
        Z = []
        for lm in landmarks:
            Z.append(((atan2(lm[0]-self.y,lm[1]-self.x)-self.orientation)+2.*pi)%(2.*pi))
        #print Z
        if (flg != 0):
            for i in range(len(Z)):
                Z[i]=(Z[i]+random.gauss(0,self.bearing_noise))%(2.*pi)
        #print 'noisy='+str(Z)
...
def generate_ground_truth(motions):
    ...
    myrobot = robot()
    myrobot.set_noise(bearing_noise, steering_noise, distance_noise) # why set the noise values for ground truth?
...
#print 'should be [x=93.476 y=75.186 orient=5.2664]'
