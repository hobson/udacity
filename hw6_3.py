...
def make_data(N, num_landmarks, world_size, measurement_range, motion_noise, 
    ...
    while not complete:
        ...
        #complete = (sum(seen) >= 0.5*num_landmarks)
...
def slam2(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        for b in range(2*world_dim): # or should this be world_dim**2 ??
            Omega.value[n+b][n+b] +=  1.0 / motion_noise
        for b in range(world_dim):
            Omega.value[n+b          ][n+b+world_dim] += -1.0 / motion_noise
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise): # N no longer needed
    world_dim = 2

    # Set the dimension of the filter
    dim = world_dim * (1 + num_landmarks)  # retain only 1 set of position coordinates
