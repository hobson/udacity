...
#
def build_road(length, lane_speeds, print_flag = True, obstacles = False, obstacle_prob = 0.05):
...
cost_step = 0 # the cost associated with making a move, regardless of move type.

OBSTACLE_COST = 1.0e3
OBSTACLE_VALUE = 0

def optimum_policy(value_grid,delta=[[-1,0],[0,-1],[1,0],[0,1]],delta_name=['^', '<', 'v', '>'],cost_step=1,init=[0,0],goal=[float('inf'), float('inf')]):
    N,M = len(value_grid),len(value_grid[0])
    init,goal = [min(max(init[0],0),N),min(max(init[1],0),M)],[min(max(goal[0],0),N),min(max(goal[1],0),M)]
    print 'value_grid'

    pprint.pprint(value_grid)
#    grid   = [[ cost_grid[row][col]>=OBSTACLE_COST?1:0 for col in range(len(grid[0]))] for row in range(len(cost_grid))]
    grid      = [[ int(value_grid[row][col]<=OBSTACLE_VALUE) for col in range(len(value_grid[0]))] for row in range(len(value_grid))]
    cost_grid = [[ 1./value_grid[row][col] if value_grid[row][col] else OBSTACLE_COST for col in range(len(value_grid[0]))] for row in range(len(value_grid))]
    # TODO value is realy cost, so somehow value = cost_grid
    value  = [[ OBSTACLE_COST                         for col in range(len(grid[0]))] for row in range(len(grid))]
    policy = [[' '                                     for col in range(len(grid[0]))] for row in range(len(grid))]
    change = True
    
    if isinstance(cost_step,(float,int)):
        cost_delta = [cost_step for i in range(len(delta))]
    elif len(cost_step)==len(delta):
        cost_delta = cost_step
    else:
        cost_delta = [cost_step[0] for i in range(len(delta))]
                ...
                for y in range(M):
                    if goal[0] == x and goal[1] == y:
                        ...
                        for a in range(len(delta)):
                            ...
                            if x2 >= 0 and x2 < N and y2 >= 0 and y2 < M and grid[x2][y2] == 0:
                                
                                # TODO: find out if cost is based on previous cell or current cell
                                v2  = value[x2][y2] + cost_grid[x2][y2] + cost_delta[a]
...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    policy,value = optimum_policy(value_grid = road,
                  delta=[[-1,1],[0,1],[+1,1]], # TODO: find out if lane changes are diagonal or vertical
                  delta_name=['^', '>', 'v'],
                  cost_step=[lane_change_cost,0,lane_change_cost],
                  init=[len(road)-1, 0],
